                ...
                <div className="grid gap-4">
                    {guides.length === 0 ? (
                        ...
                        guides.map((guide, index) => (
                            <div key={guide.id} className="group bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-lg transition-all flex items-center gap-6">
                                <div className="font-bold text-slate-300 text-xl w-8">{index + 1}</div>
                                {guide.steps.length > 0 ? (
                                    <img src={guide.steps[0].image} className="w-24 h-16 object-cover rounded-lg border border-slate-100" alt="thumb" />
                                ) : (
                                    <div className="w-24 h-16 bg-slate-100 rounded-lg flex items-center justify-center text-slate-300"><IconFileText /></div>
                                )}
